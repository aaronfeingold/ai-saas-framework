...
export const setCachedUser = async (userId: string, userData: any, ttl: number = 300) => {
  try {
...
export const getCachedSimilarDocuments = async (query: string, limit: number, userId?: string) => {
  try {
    const cacheKey = `vector:${createHash('md5').update(`${query}:${limit}:${userId || 'global'}`).digest('hex')}`
...
export const setCachedContent = async (key: string, content: any, ttl: number = 600) => {
  try {
...
export const setCachedSession = async (sessionId: string, sessionData: any, ttl: number = 3600) => {
  try {
